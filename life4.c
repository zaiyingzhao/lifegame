...
int main(int argc, char **argv) {
  ...
  if ( argc > 2 ) {
    ...
    my_init_cells(height, width, cell, NULL); // デフォルトの初期値を使う（今回はこれのみを考える）
  }
...
void my_update_cells(const int height, const int width, int cell[height][width]) {
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            if (cell[i][j] == 1) {
                if (my_count_adjacent_cells(i, j, height, width, cell) < 40) {
                    tmp_cell[i][j] = 0;
                }
                else {
                    tmp_cell[i][j] = 1;
            ...
            if (cell[i][j] == 0) {
                if (my_count_adjacent_cells(i, j, height, width, cell) % 10 >= 2 && my_count_adjacent_cells(i, j, height, width, cell) >= 10) { //ヤギと草の生存条件をどちらも満たしたときはヤギを優先
                    tmp_cell[i][j] = 1;
                }
                else if (my_count_adjacent_cells(i, j, height, width, cell) >= 30) {
                    tmp_cell[i][j] = 10;
                }
                else {
                    tmp_cell[i][j] = 0;
            ...
            if (cell[i][j] == 10) {
                if (my_count_adjacent_cells(i, j, height, width, cell) % 10 >= 2) {
                    tmp_cell[i][j] = 0;
                }
                else {
                    tmp_cell[i][j] = 10;
